...
export class MapComponent extends BaseComponent implements OnInit {
  ...
  constructor(injector: Injector, private cdr: ChangeDetectorRef, private _reportsService: ReportsService, private _branchService:BrancheService) {
      super(injector)
  ...
  getMapData(type: number = 1) {
      let filter = {
          "startDate": this.rangeDates && this.rangeDates[0] ? this.utility.convertDate(this.rangeDates[0]) : null,
          "endDate": this.rangeDates && this.rangeDates[1] ? this.utility.convertDate(this.rangeDates[1]) : null,
          "branchId": 0
      }
      this.mapLoading = true
      // @ts-ignore
      this.mapOptions.series[0].name = type
      if (type){
          // @ts-ignore
          this.mapOptions.series[0].tooltip={
            
              headerFormat: '<span style="font-size:12px;font-weight:600">{series.name}</span><br/>',
              pointFormatter: function() {
                  return 'الطلبات الناجحة: '+ this.value + " - الطلبات الملغاة: "+ this.value2;
              }
          }
      }
      // @ts-ignore
